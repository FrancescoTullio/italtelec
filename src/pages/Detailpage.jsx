...
function Detailpage() {
  const { id } = useParams(); // Prende l'ID dai parametri URL
  ...
  useEffect(() => {
    console.log('useEffect chiamato, id:', id); // Debug
    if (id) {
      fetchAntenna();
    } else {
  ...
  const fetchAntenna = async () => {
    console.log('Inizio fetch per ID:', id); // Debug
    setLoading(true);
    setError(null);

    try {
      const url = `http://localhost:3004/italtelec/${id}`;
        ...
        <div className="antenna-detail-card">
          ...
          <div className="antenna-content-section">
            ...
            <div className="antenna-footer">
              <button className="antenna-add-to-cart-button">
                <svg className="antenna-button-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
